...
class VoxtralProvider(TranscriptionProvider):
        ...
        try:
            ...
            if response.text:
                try:
                    ...
                    raise RuntimeError(f"Voxtral API request failed: {e}. Error details: {error_details}")
                except ValueError:
                    raise RuntimeError(f"Voxtral API request failed: {e}. Response: {response.text}")
            else:
    ...
    async def transcribe_stream(self, audio_stream, language: str = "en"):
        """Voxtral does not support streaming transcription."""
        raise NotImplementedError("Voxtral provider does not support streaming transcription")
